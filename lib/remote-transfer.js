...
function syncAll(nconf, setLogger, callback) {
    logger = setLogger;
    var transfers = nconf.get('remote-transfer');
...
function filterFiles1(transfer, logger, callback) {
    ...
    }).connect(utils.getConnectionParams(transfer));
}
...
//todo b2b_s1_alsb_aspect_short.time.20170227
//todo b2b_s2_cip_domainWrapper.2017-02-27
//todo jms_s1_alsb_aspect.time.part
//todo jms_s2_alsb_aspect_short.time.part
//todo b2b_s1_alsb_aspect.time.20170227
//todo cip_bpm_s1.log.20170228.08.bgz
        ...
        for (i=1; i<file.localPaths.length; i++) {
            if (fs.existsSync(file.localPaths[i])) {
                ...
                if (destReadStart != stats.size) {
                    ...
                    logger.error("Terminating processing of " + file.remotePath);
                    return callback();
                }
            }
        }
